...
def create_order_detail(order_detail_data: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return orderdetails.create_order_detail(db, order_detail_data)

@app.get("/orderdetails/", response_model=list[schemas.OrderDetail])
def read_all_order_details(db: Session = Depends(get_db)):
    return orderdetails.read_all_order_details(db)

@app.get("/orderdetails/{order_detail_id}", response_model=schemas.OrderDetail)
def read_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    order_detail = orderdetails.read_one_order_detail(db, order_detail_id)
    if order_detail is None:
        raise HTTPException(status_code=404, detail="OrderDetail not found")
    return order_detail

@app.put("/orderdetails/{order_detail_id}", response_model=schemas.OrderDetail)
def update_order_detail(order_detail_id: int, order_detail_data: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    return orderdetails.update_order_detail(db, order_detail_id, order_detail_data)

@app.delete("/orderdetails/{order_detail_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    return orderdetails.delete_order_detail(db, order_detail_id)
